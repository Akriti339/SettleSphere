...
const SimplifyCard: React.FC<SimplifyCardProps> = ({ activities, userId }) => {
  ...
  const totalAmountPayer =
    settlementDataPayer?.documents?.reduce((sum: number, settlementItem: any) => {
      return sum + parseFloat(settlementItem.Amount);
    }, 0) ?? 0;

  const totalAmountReceiver =
    settlementDataReceiver?.documents?.reduce((sum: number, settlementItem: any) => {
      return sum + parseFloat(settlementItem.Amount);
    }, 0) ?? 0;
    ...
    <>
      <div className="text-white font-bold mb-3">
        <p className="text-blue-500 text-xl">
          ...
          <span
            className={` font-bold ${
              payeer - receiver < 0 ? "text-red" : "text-purple-500"
            }`}>
            {" "}
        ...
        <p className="text-sm">
          ...
          <span style={{ display: "inline-block" }}>
            {issettlementDataPayerLoading ? (
              ...
              <span className="text-purple-500">&#8377;{payeer.toFixed(2)}</span>
            )}
          </span>
        </p>
      </div>
